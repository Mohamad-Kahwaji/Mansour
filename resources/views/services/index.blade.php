<x-layouts.site :site-settings="$siteSettings">
    <section class="bg-cream-2 pt-32 pb-16 sm:pt-40 sm:pb-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
            <span class="inline-flex items-center gap-3 font-display text-xs font-semibold tracking-[.24em] text-gold uppercase">
                <span class="h-px w-8 bg-current opacity-60"></span>
                {{ __('site.services_page.eyebrow') }}
            </span>
            <h1 class="font-display mt-5 text-3xl leading-tight font-bold tracking-tight sm:text-4xl lg:text-5xl">
                {{ __('site.services_page.heading') }}
            </h1>
        </div>
    </section>

    <section class="bg-cream-2 pb-16 sm:pb-24 lg:pb-28">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
            <div class="grid gap-px border border-cream-line bg-cream-line sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($services as $service)
                    @include('partials.site.service-card', ['service' => $service])
                @endforeach
            </div>
        </div>

        @foreach ($services as $service)
            @include('partials.site.service-image-modal', ['service' => $service])
        @endforeach
    </section>
</x-layouts.site>
