    ...
    <div class="mx-auto flex max-w-7xl items-center gap-4 px-4 py-5 sm:gap-8 sm:px-6 sm:py-6 lg:px-10">
        <a href="{{ route('home') }}" class="flex min-w-0 items-center gap-2 text-ondark sm:gap-3">
            @if ($siteSettings?->logo)
                <img src="{{ asset('storage/'.$siteSettings->logo) }}" alt="{{ $siteSettings?->getTranslation('site_name', app()->getLocale()) ?? $siteSettings?->site_name }}" class="h-8 w-auto max-w-[120px] shrink-0 object-contain sm:h-10">
            @else
            ...
            <span class="font-display truncate text-xs font-bold tracking-wide sm:text-sm">
                {{ $siteSettings?->getTranslation('site_name', app()->getLocale()) ?? $siteSettings?->site_name }}
            </span>
        </a>
